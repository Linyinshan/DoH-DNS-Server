...
public class DnsHandler implements Runnable {
    ...
    private static volatile long lastSwitchQueryCount = 0;
    ...
    private synchronized void initializeDohServers() {
        ...
        if (!configuredServers.isEmpty()) {
            ...
            // 修复：始终使用配置的 autoswitch 设置，不根据服务器数量强制启用
            autoSwitchEnabled = config.isAutoSwitchEnabled();
            logger.info("配置了{}个DoH服务器，自动切换：{}", 
                       configuredServers.size(), autoSwitchEnabled ? "启用" : "禁用");
        } else {
            // 使用内置服务器列表
            dohServerList.clear();
     ...
     * 重置切换计数的方法
    */
    public static synchronized void resetSwitchCounter() {
        lastSwitchQueryCount = dohQueryCount;
    }
    ...
    private void checkAndSwitchDohServer() {
        if (autoSwitchEnabled && dohServerList.size() > 1) {
            long sinceLastSwitch = dohQueryCount - lastSwitchQueryCount;
            if (sinceLastSwitch >= switchThreshold) {
                switchToNextServer(false);
                lastSwitchQueryCount = dohQueryCount; // 更新上次切换计数
                logger.debug("自动切换服务器，自上次切换后处理了 {} 个查询", sinceLastSwitch);
            }
        }
    }
    ...
    public static synchronized void reinitializeFromConfig(DnsConfig config) {
        try {
            // 清空当前服务器列表
            dohServerList.clear();
            ...
            logger.info("配置重载：服务器列表已更新，共{}个服务器，自动切换：{}", 
                       dohServerList.size(), autoSwitchEnabled ? "启用" : "禁用");
        } catch (Exception e) {
            logger.error("重新初始化服务器列表失败", e);
        }
    }
